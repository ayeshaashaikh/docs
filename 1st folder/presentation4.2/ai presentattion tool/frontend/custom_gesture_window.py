...
def check_gesture_exists(action):
    """Check if a gesture for the given action already exists in the database."""
    try:
        conn = mysql.connector.connect(**DB_CONFIG)
        cursor = conn.cursor()
        query = "SELECT COUNT(*) FROM gestures WHERE action = %s"
        ...
        print(f"Error checking gesture existence: {e}")
        return False
...
def record_and_save_gesture(action):
    ...
    First checks if the gesture already exists.
    """
    # Check if gesture already exists
    if check_gesture_exists(action):
        messagebox.showwarning(
            "Gesture Already Exists", 
            f"A gesture for '{action}' is already saved in the database.\n\n"
            f"Please delete the existing gesture first using 'Manage Gestures' "
            f"before recording a new one."
        )
        return
